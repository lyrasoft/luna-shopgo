...
import { data, slideDown, slideUp } from '@windwalker-io/unicorn-next';
...
const items = ref(
  uniqueItemList(props.variants).map((item) => {
    item.attachment =
      item.attachment || {
        method: 'offsets',
        price: 0,
...
function toggleAll(el: HTMLInputElement) {
...
const checks = computed(() => {
  return items.value.filter((item) => Number(item.attachment.state) === 1).length;
});

function onMethodChange(item: ProductVariant) {
  syncAllFields(item.attachment.method, 'method');

  normalizePricing(item);
}

function onMaxQuantityChange(item: ProductVariant) {
...
function normalizePricing(item: ProductVariant) {
  if (
...
function getPriceStep(item: ProductVariant) {
  if (item.attachment.method === 'percentage') {
    return '1';
  }

  return data('price.step') || '0.0001';
}

// Open / Close
const variantList = ref<HTMLDivElement>();

watch(open, (v) => {
  setTimeout(() => {
    if (v) {
      slideDown(variantList.value!);
    } else {
      slideUp(variantList.value!);
    }
  }, 0);
}, { immediate: true });
