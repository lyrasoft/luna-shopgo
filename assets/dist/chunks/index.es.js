...
class h {
  static hashStr(f, n = !1) {
    return this.onePassHasher.start().appendStr(f).end(n);
  }
  static hashAsciiStr(f, n = !1) {
    return this.onePassHasher.start().appendAsciiStr(f).end(n);
  }
  ...
  appendStr(f) {
    ...
    let t = this._bufferLength, e, r;
    for (r = 0; r < f.length; r += 1) {
      if (e = f.charCodeAt(r), e < 128)
        n[t++] = e;
      else if (e < 2048)
        ...
        if (e = (e - 55296) * 1024 + (f.charCodeAt(++r) - 56320) + 65536, e > 1114111)
          throw new Error(
            "Unicode standard supports code points up to U+10FFFF"
          );
        n[t++] = (e >>> 18) + 240, n[t++] = e >>> 12 & 63 | 128, n[t++] = e >>> 6 & 63 | 128, n[t++] = e & 63 | 128;
      }
      t >= 64 && (this._dataLength += 64, h._md5cycle(this._state, s), t -= 64, s[0] = s[16]);
    }
  ...
  appendAsciiStr(f) {
    const n = this._buffer8, s = this._buffer32;
    let t = this._bufferLength, e, r = 0;
    for (; ; ) {
      for (e = Math.min(f.length - r, 64 - t); e--; )
        n[t++] = f.charCodeAt(r++);
      if (t < 64)
        break;
      this._dataLength += 64, h._md5cycle(this._state, s), t = 0;
    }
    return this._bufferLength = t, this;
  }
  /**
   ...
   */
  setState(f) {
    const n = f.buffer, s = f.state, t = this._state;
    let e;
    for (this._dataLength = f.length, this._bufferLength = f.buflen, t[0] = s[0], t[1] = s[1], t[2] = s[2], t[3] = s[3], e = 0; e < n.length; e += 1)
   ...
   */
    ...
    const n = this._bufferLength, s = this._buffer8, t = this._buffer32, e = (n >> 2) + 1;
    this._dataLength += n;
    const r = this._dataLength * 8;
    if (s[n] = 128, s[n + 1] = s[n + 2] = s[n + 3] = 0, t.set(h.buffer32Identity.subarray(e), e), n > 55 && (h._md5cycle(this._state, t), t.set(h.buffer32Identity)), r <= 4294967295)
      t[14] = r;
    else {
      const i = r.toString(16).match(/(.*?)(.{0,8})$/);
      if (i === null) return f ? o : "";
      const u = parseInt(i[2], 16), c = parseInt(i[1], 16) || 0;
      t[14] = u, t[15] = c;
    }
    return h._md5cycle(this._state, t), f ? this._state : h._hex(this._state);
  }
}
